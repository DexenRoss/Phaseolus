...
type PublicationAuthorRelation = {
  id: string;
  authorId: string;
  authorOrder: number;
...
type PublicationData = {
  ...
  authors: PublicationAuthorRelation[];
};
...
function formatDateForInput(dateValue: Date | string | null) {
  if (!dateValue) return "";
...
export default function EditPublicationForm({ publication, authors }: Props) {
  ...
  const [publicationDate, setPublicationDate] = useState(
    formatDateForInput(publication.publicationDate)
  );
  const [status, setStatus] = useState<
    "DRAFT" | "PUBLISHED" | "ARCHIVED"
  >(publication.status);
  const [authorIds, setAuthorIds] = useState<string[]>(
    publication.authors.map((item) => item.authorId)
  );
